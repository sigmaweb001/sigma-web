...
const { data: pinResource } = await useAsyncData(
  `production-pin-resource:${appConfig.productPinResource.path}`,
  () => queryCollection('resources').where('path', '=', appConfig.productPinResource.path).first(),
  { immediate: appConfig.productPinResource.enabled },
)
...
<template>
  ...
  >
    ...
    <UNavigationMenu
      ...
      :ui="{
        viewport: 'lg:w-(--reka-navigation-menu-viewport-width)',
        childList: 'lg:w-128',
    ...
    >
      <template #products-content="{ item: products }">
        <div
          class="lg:w-256 grid"
          :class="[hasFeature ? 'grid-cols-[minmax(0,2fr)_minmax(0,.75fr)]' : 'grid-cols-[minmax(0,1fr)_minmax(0,1fr)]']"
        >
          ...
          >
            <div>
              ...
              <ResourceItemImp
                v-if="pinResource.type === 'resource'"
                hide-dir
                class="h-full"
                :item="pinResource"
              />
