<script lang="ts" setup>
const props = defineProps<{
  item: any
  hideAuthor?: boolean
  hideDir?: boolean
}>()

const { item, hideAuthor, hideDir } = toRefs(props)

const appConfig = useAppConfig()

const author = computed(() => appConfig.authors.find(a => a.slug === item.value.author))
const dirPath = computed(() => {
  const pathArr = item.value.path.split('/')
  return pathArr.slice(0, pathArr.length - 1).join('/')
})

const { data: dataDir } = await useAsyncData(
  `resources-dir-blogs`,
  () => queryCollection('content').path('/resources/blogs').first(), { watch: [dirPath] },
)

const NuxtLink = resolveComponent('NuxtLink')
const localePath = useLocalePath()
</script>

<template>
  <div class="flex flex-col gap-2 overflow-hidden bg-resource">
    <NuxtLink
      :to="localePath(item.path)"
      class="relative block aspect-16/9 overflow-hidden rounded-12px bg-gray-200 transition-all duration-300 hover:scale-105"
    >
      <template v-if="item?.thumbnail">
        <img
          :src="item.thumbnail"
          class="absolute inset-0 size-full object-cover"
        >
      </template>
      <template v-else>
        <div class="absolute inset-0 size-full flex items-center justify-center">
          <Icon
            :name="item.icon ?? 'i-ri:image-fill'"
            class="h-50% w-50% text-primary/75"
          />
        </div>
      </template>
    </NuxtLink>
    <div
      v-if="!hideDir"
      class="px-3"
    >
      <span class="inline rounded-xl bg-gray-200 px-2 py-1 text-xs font-medium">
        {{ dataDir?.title }}
      </span>
    </div>

    <div class="h-50px px-3">
      <NuxtLink
        :to="localePath(item.path)"
        class="flex-1 text-lg font-semibold leading-snug tracking-tight dark:text-white"
      >
        <span
          class="transition-[background_size] line-clamp-2 cursor-pointer bg-[length:0px_10px] bg-gradient-to-r bg-left-bottom bg-no-repeat duration-500 hover:(text-primary underline)"
        >
          {{ item.title }}
        </span>
      </NuxtLink>
    </div>

    <div class="h-40px px-3">
      <span class="line-clamp-2 text-sm">
        {{ item.description }}
      </span>
    </div>
    <div class="flex flex-1 flex-col justify-end">
      <div
        v-if="author && !hideAuthor"
        class="mb-2 w-full flex items-center justify-between gap-3 px-3"
      >
        <div class="flex flex-grow items-center gap-2">
          <div class="relative h-32px w-32px flex-shrink-0">
            <img
              class="absolute inset-0 h-full w-full rounded-full object-cover"
              :src="author.avatar"
              :alt="author.slug"
            >
          </div>
          <span class="line-clamp-1 max-w-1/2 min-w-0 text-sm">
            {{ author.name }}
          </span>
        </div>
        <p
          v-if="author.title"
          class="line-clamp-1 my-0 max-w-1/4 min-w-0 text-xs"
        >
          {{ author.title }}
        </p>
      </div>
      <div
        v-if="hideAuthor"
        class="mb-2 mt--2 flex items-end justify-between px-3"
      >
        <UButton
          :as="NuxtLink"
          variant="link"
          class="text-sm p-0!"
          :to="localePath(item.path)"
        >
          Read more
          <div class="i-ri:arrow-right-line ml-1 size-16px!" />
        </UButton>
      </div>
    </div>
  </div>
</template>
